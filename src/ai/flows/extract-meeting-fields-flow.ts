...
export const extractMeetingFieldsFlow = ai.defineFlow(
  ...
  async input => {
    const { output } = await ai.generate({
      ...
      Only return values for fields that you can extract from the user's message. If a field is not present or implied in the message, do NOT include it. A full name requires at least two name tokens: do not return firstName or lastName from a single-token name. Use standard formats (e.g. ISO date for preferredDate [YYYY-MM-DD], HH:MM for preferredTime, IANA string for timezone). For reasonForMeeting, turn a long or informal explanation into a concise, professional summary of at most two sentences without changing its meaning.`,
