...
import { X, Bot, HelpCircle, ChevronDown, PanelRightOpen, PanelLeftOpen } from 'lucide-react';
      ...
      <header className="p-4 flex items-center justify-between border-b shrink-0">
        <div className="flex-1 min-w-0">
          ...
          <div className="text-sm text-muted-foreground flex items-center gap-2 mt-1">
            <Badge variant={blog.tag === 'Paid' ? 'destructive' : 'secondary'} className="capitalize">{blog.tag}</Badge>
        ...
        <div className="flex items-center gap-2">
            <Button variant="ghost" size="icon" onClick={() => setIsAiPanelOpen(!isAiPanelOpen)} data-cursor-hover>
                {isAiPanelOpen ? <PanelRightOpen size={20} /> : <PanelLeftOpen size={20} />}
            </Button>
      ...
      <div className="flex-grow flex overflow-hidden">
        ...
        {isAiPanelOpen && (
            <motion.aside 
                ...
                className="w-full md:w-80 border-l p-4 shrink-0 flex flex-col gap-4 overflow-hidden"
            >
            <ScrollArea className="h-full">
