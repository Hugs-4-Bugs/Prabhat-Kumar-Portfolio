...
export function BlogCard({ blog, onRead, onBookmark, isBookmarked }: BlogCardProps) {
  return (
    <motion.div
      whileHover="hover"
      className="group relative h-full rounded-2xl overflow-hidden border border-slate-700/50 bg-slate-900/50 backdrop-blur-xl transform-style-3d"
    >
      {/* Animated Border */}
      ...
      <div className="p-6 h-full flex flex-col">
        ...
        <div className="flex justify-between items-center mt-auto">
          ...
          <button onClick={() => onBookmark(blog.slug)} className="p-2 rounded-full hover:bg-slate-700/50 transition-colors">
            <Bookmark
              className={`h-5 w-5 transition-colors ${
                isBookmarked ? 'text-yellow-400 fill-yellow-400/20' : 'text-slate-400 hover:text-white'
              }`}
            />
          </button>
        </div>
